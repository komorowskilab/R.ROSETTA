saveLineByLine <- function(rules, path, discrete=FALSE, filterByPval=FALSE, pval=0.05)
{
  if(discrete)
  {
    ...
    if(filterByPval){
      dflbl<-cbind(lst5,rules["decision"],rules["accuracyRHS"],rules["supportRHS"],rules["coverageRHS"],rules["pValue"],row.names = NULL)[which(rules["pValue"]<pval),]
    }else
    {
      dflbl<-cbind(lst5,rules["decision"],rules["accuracyRHS"],rules["supportRHS"],rules["coverageRHS"],rules["pValue"],row.names = NULL)
    }
  }
  else{
